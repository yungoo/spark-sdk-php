...
if ($err == null) {
    if (count($ret['data']["products"]) > 1) {
        ...
        if (count($cidrBlocks) > 0) {
            $rules[] = array("cidrBlock" => $cidrBlocks[0]["cidr"], "count" => 2);
        }
        ...
        //     $rules[] = array("exclude" => false, "cidrBlock" => $cidrBlocks[0]["cidr"], "count" => 1);
        //     $rules[] = array("exclude" => true, "cidrBlock" => $cidrBlocks[1]["cidr"], "count" => 1);
        // }

        // # CASE-2: 从指定IP段抽取指定数量段IP，数量不对： order item quantity is inconsistent with cidr rules
        // if (count($cidrBlocks) > 0) {
        //     $rules[] = array("cidrBlock" => $cidrBlocks[0]["cidr"], "count" => 1);
        // }
        ...
        // if (count($cidrBlocks) > 0) {
        //     $rules[] = array("cidrBlock" => $cidrBlocks[0]["cidr"], "count" => 1);
        //     $rules[] = array('exclude'=>True, "cidrBlock" => $cidrBlocks[0]["cidr"], "count" => 1);
        // }
